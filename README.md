...
// FIXED: Implemented the 2D matrix addition kernel
__global__ void sumMatrixOnGPU2D(float *A, float *B, float *C, int NX, int NY)
{
    ...
    // Map 2D coordinates to a flat 1D global array index
    unsigned int idx = iy * NX + ix;

    // Boundary check to protect against illegal memory configurations
    if (ix < NX && iy < NY)
    {
        C[idx] = A[idx] + B[idx];
    }
...
int main(int argc, char **argv)
{
    ...
    // FIXED: Added grid and block into the execution configuration
...
## OUTPUT:
<img width="1464" height="465" alt="image" src="https://github.com/user-attachments/assets/adabf04d-3201-49ee-98ee-00143996ef31" />


## RESULT:
The host took 0.858499  seconds to complete it’s computation, while the GPU outperforms the host and completes the computation in 0.098715 seconds. Therefore, float variables in the GPU will result in the best possible result. Thus, matrix summation using 2D grids and 2D blocks has been performed successfully.
